...
export default {
    ...
    methods: {
        ...
        reconstructData() {
            ...
            tags = tags.map(e => { return e.toLowerCase(); })
        ...
        clickTag(tag) {
            // check if tag exist in filter_tag
            if (tag == 'all') {
                this.filter_tags = ['all'];
            } else {
                this.filter_tags = this.filter_tags.filter((e) => { return e !== 'all' });
...
.tag {
    border: rgb(252, 239, 194) solid 1px;
    border-radius: 4px;
    position: absolute;
    top: -6px;
    left: 45px;
